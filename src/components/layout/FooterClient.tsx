...
export function FooterClient({ bookCount }: { bookCount: number }) {
  const { ref, visible } = useFadeIn();

  const countLabel = bookCount > 0
    ? `${bookCount.toLocaleString()} Public Domain Books`
    : "Public Domain Books";
        ...
        <div className="lg:hidden space-y-8">
          <div className="space-y-4">
            <Link href="/" className="inline-flex items-center gap-3" aria-label="LamboReads home">
              <img src="/images/logo.png" alt="LamboReads" className="h-10 w-10 object-contain" />
              <span className="text-[17px] font-bold tracking-tight text-white">LamboReads</span>
            </Link>
