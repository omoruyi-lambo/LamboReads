...
const CONFIG = BUCKET_CONFIGS["samples"];

interface Props {
  /** The book's UUID — used to build the storage path samples/{bookId}/… */
  bookId: string;
  initialUrl?: string | null;
  initialName?: string | null;
  onUploaded: (url: string, storagePath: string, fileName: string) => void;
  onRemoved?: () => void;
  className?: string;
}
...
export function SampleUpload({
  ...
  const processFile = useCallback(
    async (file: File) => {
      ...
      if (result) {
        ...
        onUploaded(result.url, result.storagePath, file.name);
      }
    },
    [upload, onUploaded]
  );
  ...
  const handleRemove = async () => {
    if (existingUrl && state.storagePath) {
      await deleteFile(state.storagePath);
    }
    setCurrentFile(null);
    setExistingUrl(null);
  ...
  const hasFile = !!(existingUrl || isUploading);
    ...
    <div className={cn("space-y-3", className)}>
        ...
        >
          ...
          <div className="text-center">
            <p className="text-sm font-medium text-[#111827]">
              {isDragActive ? "Drop the sample here" : "Drag & drop a sample / preview file"}
            </p>
      ...
      {!isUploading && existingUrl && (
        <div className="flex items-center gap-3 rounded-xl border border-[#D1FAE5] bg-[#ECFDF5] p-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-[#111827] truncate">
              {existingName ?? "Sample file uploaded"}
            </p>
            {fileExt && (
