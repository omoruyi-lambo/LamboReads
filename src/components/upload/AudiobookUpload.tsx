...
const CONFIG = BUCKET_CONFIGS["audiobooks"];

interface Props {
  /** The book's UUID — used to build the storage path audiobooks/{bookId}/… */
  bookId: string;
...
export function AudiobookUpload({
  ...
  const { state, upload, retry, reset, deleteFile, getSignedUrl } = useUpload({
    bucket: "audiobooks",
    entityId: bookId,
  });
  ...
  const handlePreview = async () => {
    const path = existingPath ?? state.storagePath;
    if (!path) return;
    // Generate a 1-hour signed URL for in-browser audio preview
    ...
    <div className={cn("space-y-3", className)}>
      ...
      {isUploading && currentFile && (
        <div className="rounded-xl border border-[#E5E7EB] bg-white p-4 space-y-3">
          ...
          <p className="text-[10px] text-[#94A3B8]">
            Large audio files may take a few minutes. Please keep this page open.
          </p>
        </div>
      )}
      ...
      {!isUploading && existingPath && (
        <div className="rounded-xl border border-[#D1FAE5] bg-[#ECFDF5] p-4 space-y-3">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-[#111827] truncate">
                {existingName ?? "Audiobook file uploaded"}
              </p>
              {fileExt && (
            ...
            <button
              ...
              aria-label="Remove audiobook file"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
          ...
          {previewUrl ? (
            <audio
              ...
              aria-label="Audiobook preview"
            />
          ) : (
            <button
              type="button"
              onClick={handlePreview}
              className="text-xs font-medium text-[#059669] underline underline-offset-2 hover:text-[#047857]"
            >
              Preview audio
            </button>
          )}
